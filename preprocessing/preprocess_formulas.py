...
import sys, os, argparse, logging, subprocess, shutil, csv
...
def is_ascii(str):
    try:
        str.decode('ascii')
        return True
    except UnicodeError or AttributeError:
        return False
...
def main(args):
    ...
    if parameters.input_file[-4:] == ".csv":
        with open(parameters.input_file) as f:
            reader = csv.reader(f)
            with open(parameters.input_file[:-4] + ".txt", "w") as ff:
                for line in reader:
                    if not "formula" in line[0]:
                        ff.write(line[0] + "\n")
        parameters.input_file = parameters.input_file[:-4] + ".txt"
    logging.basicConfig(
        level=logging.INFO,
    ...
    with open(temp_file, 'w') as fout:
        with open(output_file, 'rb') as fin:
            for line in fin:
                try:
                    fout.write(line.decode('utf-8').replace('\r', ' ').strip() + '\n')  # delete \r
                except UnicodeDecodeError:
                    fout.write('\n')

    cmd = "cat %s | node preprocessing/preprocess_latex.js %s > %s " % (temp_file, parameters.mode, output_file)
    ret = subprocess.call(cmd, shell=True)
    os.remove(temp_file)
    if ret != 0:
        logging.error('FAILED: %s' % cmd)
